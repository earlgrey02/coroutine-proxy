...
이 레포지토리는 코루틴의 내부 원리를 통해 일시 중지 함수에 동적 프록시를 적용한 예제이다.
...
JVM 진영에서 동적 프록시는 일반적으로 JDK Dynamic Proxy와 CGLib(Code Generator Library) 등을 통해 구현한다.

### Dynamic Proxy with Coroutine

기본적으로 JDK Dynamic Proxy나 CGLib(Code Generator Library)는 코루틴의 일시 중지 함수에 대한 프록시를 지원하지 않는다.
...
실제로 JDK Dynamic Proxy의 `InvocationHandler`에서는 `invoke()`가 일시 중지 함수가 아니므로 내부에서 코루틴을 사용할 수 없다.<br/>
...
본 예제는 JDK Dynamic Proxy의 `InvocationHandler`와 CGLib의 `MethodInterceptor`에 대한 코루틴 변형만 구현되어 있다.<br/>
